# taken directly from https://github.com/vincent-leguen/DILATE
...
class PathDTWBatch(Function):
    ...
    def forward(ctx, D, gamma):  # D.shape: [batch_size, N , N]
        ...
        grad_gpu = torch.zeros((batch_size, N, M)).to(device)
        Q_gpu = torch.zeros((batch_size, N + 2, M + 2, 3)).to(device)
        E_gpu = torch.zeros((batch_size, N + 2, M + 2)).to(device)
    ...
    def backward(ctx, grad_output):
        ...
        batch_size, N, M = D_cpu.shape
        Hessian = torch.zeros((batch_size, N, M)).to(device)
        for k in range(0, batch_size):
            _, hess_k = dtw_hessian_prod(D_cpu[k, :, :], Z[k, ...], Q_cpu[k, :, :, :], E_cpu[k, :, :], gamma)
